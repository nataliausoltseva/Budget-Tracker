import React, { useContext } from 'react';
import { StyleProp, TouchableOpacity, ViewStyle } from 'react-native';
import Svg, { Path } from 'react-native-svg';
import { AppContext } from '../context/AppContext';

type Props = {
    style?: StyleProp<ViewStyle> | undefined,
    onPress?: () => void,
    isChecked?: boolean,
}

const CheckboxIcon = ({ style, onPress, isChecked = false }: Props) => {
    const appState = useContext(AppContext);
    return (
        <TouchableOpacity style={[{ borderColor: appState.isDarkMode ? "white" : 'black', borderWidth: 1 }, style]} onPress={onPress}>
            <Svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill={'none'}
            >
                <Path
                    d="M10.2426 16.3137L6 12.071L7.41421 10.6568L10.2426 13.4853L15.8995 7.8284L17.3137 9.24262L10.2426 16.3137Z"
                    fill={isChecked ? appState.isDarkMode ? "#A78DFF" : "#01B0E6" : 'none'}
                />
                <Path
                    d="M1 5C1 2.79086 2.79086 1 5 1H19C21.2091 1 23 2.79086 23 5V19C23 21.2091 21.2091 23 19 23H5C2.79086 23 1 21.2091 1 19V5ZM5 3H19C20.1046 3 21 3.89543 21 5V19C21 20.1046 20.1046 21 19 21H5C3.89543 21 3 20.1046 3 19V5C3 3.89543 3.89543 3 5 3Z"
                    fill="none"
                />
            </Svg>
        </TouchableOpacity>
    );
};

export default CheckboxIcon;